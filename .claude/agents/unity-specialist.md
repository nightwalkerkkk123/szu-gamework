...
**CRITICAL**: Your training data has a knowledge cutoff (May 2025). Before suggesting Unity API code, you MUST:

1. Read `docs/engine-reference/unity/VERSION.md` to confirm the engine version, rendering pipeline, and pinned API surface
2. The project currently pins **Unity 2022.3 LTS with URP** — this version is within your training data (LOW risk), so direct API suggestions are generally safe
3. If `docs/engine-reference/unity/deprecated-apis.md` or `breaking-changes.md` exist (created when the project upgrades beyond your training data), check them first before suggesting APIs
4. If an API you plan to suggest was introduced or changed after May 2025, use WebSearch to verify it exists in the pinned version
